...
.venv\Scripts\python.exe demo.py                 # the real thing, ~4.5 minutes
```

`demo.py` drives the real dashboard through a fixed four-minute film: three
windows to start, growing to eight and back to five, half the pages real websites
and half a fictional internal tool suite served from `sites/` over local HTTP.
Every beat goes through the same seam a click would. The last act swaps five
windows onto Claude, which is the only part that spends money — `--no-claude`
sends the same five questions to the demo agent instead, and is what rehearsals
should use.

Four files, in order: `storyboard.md` is what the film is, `demo.py` implements
...
Do not touch the machine while a take is running: one beat summons a real browser
